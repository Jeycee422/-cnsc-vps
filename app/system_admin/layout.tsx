...
export default function SystemAdminLayout({
  ...
  useEffect(() => {
    // If truly no token, send to sign-in
    if (!token) {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (role !== 'system_admin') {
            // Route users with other roles to their respective areas instead of sign-in
            if (role === 'admin' || role === 'super_admin') {
              window.location.href = '/admin/dashboard';
            } else if (role === 'security_guard') {
              window.location.href = '/security/scans';
            } else {
              window.location.href = '/user/home';
            }
          }
        }
        // For non-OK (but not auth) responses, avoid redirect loops; leave user on page.
      } catch (e: any) {
        if (e?.name === 'AbortError') return;
        // Network/other error: do not redirect to avoid loop; keep layout visible
      }
    })();
  ...
  useEffect(() => {
    ...
    socket.on('admin:notification', ({ id, message, ts }: { id: string; message: string; ts?: string }) => {
      setNotifications(prev => [
        { id, message, ts: ts || new Date().toISOString() },
        ...prev
      ].slice(0, 20));
    });
  ...
  const navigation = [
    {
      ...
      href: '/system_admin/security_guards',
      icon: (
      ...
      <div className={`fixed inset-y-0 left-0 z-50 w-64 bg-[#7E0303] text-white transition-transform duration-300 transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex items-center justify-between h-16 px-4 border-b border-[#5E0202]">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-lg font-semibold">System Admin</span>
          </div>
          <button
